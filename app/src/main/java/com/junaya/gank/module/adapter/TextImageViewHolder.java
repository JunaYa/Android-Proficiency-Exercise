...
public class TextImageViewHolder extends BaseViewHolder<Gank> {
    ...
    public void bindViewHolder(Gank gank) {
        ...
        ImagesAdapter imagesAdapter = new ImagesAdapter(gank);
...
//        }
        private static Handler mWeakReference;
        private static ViewPager mPagerProvider;

        public GankRunnable(Handler weakReference, ViewPager viewHolder) {
            mWeakReference = weakReference;
            mPagerProvider = viewHolder;
        }
...
//            mWeakReference.get().postDelayed(this, 5000);
            int currentPos = mPagerProvider.getCurrentItem();
            int count = mPagerProvider.getAdapter().getCount();
            int nextPos = (currentPos + 1) % count;
            mPagerProvider.setCurrentItem(nextPos, false);
            mWeakReference.postDelayed(this, 5000);
        }
    }
